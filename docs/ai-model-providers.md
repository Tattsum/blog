...
- [x] `TextGenerator` IF の定義（`internal/application/ai`）。リクエスト/レスポンスは現状プロンプト文字列のみで十分のため、拡張時に `GenerateRequest` 等を追加する。
- [x] 現行 `vertexai.Client` を `application/ai.VertexGemini` アダプタとしてラップ（`AIServer` は `ai.TextGenerator` のみに依存）
- [x] **Vertex Partner（Claude）1 本**: `application/ai.VertexClaude` — `github.com/anthropics/anthropic-sdk-go` + `vertex.WithCredentials`（ADC）。`AI_PROVIDER=vertex-claude` のとき Gemini より優先。モデルは `VERTEX_CLAUDE_MODEL`（省略時 `claude-sonnet-4-5-20250929`）。リージョン・モデルは GCP での提供状況に依存。
- [ ] その他 Partner / MaaS（DeepSeek 等）の追加アダプタ
