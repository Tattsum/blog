...
5. **Build command と Deploy command（OpenNext / Cloudflare Workers の場合・重要）**  
   ...
   - **Deploy command**: 必ず **`npm run deploy`** を指定する。  
     - `npm run deploy` は `opennextjs-cloudflare build`（Next.js ビルド＋`.open-next` 生成）の後に `opennextjs-cloudflare deploy --env production` を実行する（`wrangler.jsonc` の **`env.production`** の vars を本番に適用するため）。  
